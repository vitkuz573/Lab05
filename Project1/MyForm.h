...
#pragma endregion
		private: System::Void solution_button_Click(System::Object^ sender, System::EventArgs^ e) {
			...
			double average_call_time = System::Convert::ToDouble(average_call_time_numericupdown->Text); /* средняя продолжительность обслуживания */

			double service_flow_intensity = 1 / average_call_time; /* интенсивность потока обслуживания */
			double relative_throughput = service_flow_intensity / (intensity + service_flow_intensity); /* относительная пропускная способность */
			double absolute_throughput = intensity * relative_throughput; /* абсолютная пропускная способность */
			double channel_occupancy_probability = 1 - relative_throughput; /* вероятность занятости канала */
			double number_of_requests_served = absolute_throughput * 60; /* число обслуженных заявок */
			double number_of_requests_unserved = intensity * channel_occupancy_probability * 60; /* число необслуженных заявок */
			double rated_wsc_capacity = 60 / average_call_time; /* номинальная производительность СМО */
			double actual_wsc_capacity =  100 - (number_of_requests_served / rated_wsc_capacity * 100); /* фактическая производительность СМО */
